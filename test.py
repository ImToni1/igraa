...
def load_characteristics():
    with open("saved_hand.json", "r", encoding="utf-8") as file:
...
def game_loop():
    ...
    character_data = load_characteristics()
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_h:
                    show_characteristics = not show_characteristics
        ...
        if show_characteristics:
            y_offset = 10
            for kar in character_data:
                text_surface = font.render(f"{kar}", True, (255, 255, 255))
